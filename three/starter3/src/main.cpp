...
namespace
{
    ...
    vector<ParticleSystem * >system; //stores all particlesystems
  ...
  void initSystem(int argc, char * argv[])
  {
    ...
    srand( time( NULL ) );
    	...
    	system.push_back(new SimpleSystem());
    	system.push_back(new PendulumSystem(4));
    	system.push_back(new ClothSystem(10,10));
	...
	else	
	{
		...
		{
			cout << "invalid selection, using Rk4" <<endl; 
	    		timeStepper = new RK4();
		}
		if(argc==3)
		{
			timestep=atof(argv[2]); 
			cout << "step size is " << timestep<< endl;
		}
	}
  ...
  void stepSystem()
  {
	...
	for(int i=0; i<3; i++) 
      		timeStepper->takeStep(system[i],h);
      	//timeStepper->takeStep(system[1],h);
	//cout << "took step" <<endl; 
    }
  }
  ...
  void drawSystem()
  {
	...
	case 'f':
	{
		//we are going to flap the cloth
		if(flap==1)
			flap=0;
		else
			flap=1; 
		cout << "flaping state "<<flap <<endl;
		(*system[2]).setExternal(flap); 
	}
        default:
